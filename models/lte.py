    ...
    def query_rgb(self, coord, cell=None):
        ...
        for vx in vx_lst:
            for vy in vy_lst:
                ...
                # basis generation
                bs, q = coord.shape[:2]
                q_freq = torch.stack(torch.split(q_freq, 2, dim=-1), dim=-1) #先沿着最后一个维度拆分成2个，再沿着最后一个维度，+1维度 1，2，4，4 -> 1,2,4,2,2
                q_freq = torch.mul(q_freq, rel_coord.unsqueeze(-1)) #先扩展最后一维，再逐元素相乘
                q_freq = torch.sum(q_freq, dim=-2) #倒数第二个维度的维度大小会被压缩为 1，全部相加
