...
class SRNO(nn.Module):
    ...
    def __init__(self, encoder_spec, width=256, blocks=16):
        ...
        self.fc1 = nn.Conv2d(self.width, 256, 1) #把mlp换车conv
    ...
    def query_rgb(self, coord, cell):
        ...
        for vx in vx_lst:
            for vy in vy_lst:
                ...
                area = torch.abs(rel_coord[:, 0, :, :] * rel_coord[:, 1, :, :]) #第一个维度（x 坐标），面积
        ...
        for index, area in enumerate(areas):
            feat_s[index] = feat_s[index] * (area / tot_area).unsqueeze(1) #特征是长型的

        grid = torch.cat([*rel_coords, *feat_s, \
                          rel_cell.unsqueeze(-1).unsqueeze(-1).repeat(1, 1, coord.shape[1], coord.shape[2])], dim=1)
        show_feature_map(grid)

        x = self.conv00(grid, layer='conv00') #Lifting
        x = self.conv0(x, 0) #使用1X1conv，inspired by Ltep
        x = self.conv1(x, 1)

        feat = x
        ret = self.fc2(F.gelu(self.fc1(feat))) #projection functions

        ret = ret + F.grid_sample(self.inp, coord.flip(-1), mode='bilinear', \
                                  padding_mode='border', align_corners=False) #双线性插值
        return ret
