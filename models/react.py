...
from tool.func import PosEncoding_sin, PosEncoding_gao
...
class react(nn.Module):
    ...
    def __init__(self, encoder_spec, imnet_spec=None,
                 ...
                 in_features=2, sidelength=512, fn_samples=None, use_nyquist=True, use_pos_enc=True):
        super().__init__()
        ...
        self.positional_encoding = PosEncoding_gao(in_features=in_features,
        ...
        for vx in vx_lst:
            for vy in vy_lst:
                ...
                if self.use_pos_enc:
                    ...
                    rel_coord = torch.cat([rel_coord, rel_coord_], dim=-1)
                ...
                bs, q = coord.shape[:2]
                pred = self.imnet(inp.view(bs * q, -1), rel_coord.view(bs * q, -1)).view(bs, q, -1)  # 把坐标和查询特征表示一同送入MLP
